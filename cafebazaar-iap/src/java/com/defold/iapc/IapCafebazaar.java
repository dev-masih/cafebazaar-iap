...
public class IapCafebazaar implements Handler.Callback {
    ...
    public static final String PARAM_MESSENGER = "com.defold.iapc.messenger";
    ...
    public static final String TAG = "iapc";
    ...
    public IapCafebazaar(Activity activity, boolean autoFinishTransactions) {
    ...
    private void buyProduct(final String product, final String type, final IPurchaseListener listener) {
        this.activity.runOnUiThread(new Runnable() {
            ...
            public void run() {
                init();
                IapCafebazaar.this.purchaseListener = listener;
                Intent intent = new Intent(activity, IapCafebazaarActivity.class);
                intent.putExtra(PARAM_MESSENGER, messenger);
                intent.putExtra(PARAM_AUTOFINISH_TRANSACTIONS, IapCafebazaar.this.autoFinishTransactions);
    ...
    public void finishTransaction(final String receipt, final IPurchaseListener listener) {
        if(IapCafebazaar.this.autoFinishTransactions) {
            return;
        }
        this.activity.runOnUiThread(new Runnable() {
            @Override
